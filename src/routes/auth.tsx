  ...
  head: () => ({
    meta: [
      { title: "Sign in — Learnova" },
      {
        name: "description",
        content:
          "Quick sign-up for CBU students — just your name, student number and programme. No email verification.",
      },
    ],
  }),
  component: Auth,
...
const studentEmail = (sn: string) =>
  `s${sn
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "")}@student.learnova.app`;

type Mode = "student-signin" | "student-signup";
...
function Auth() {
  ...
  async function handleStudentSignIn(e: React.FormEvent) {
    ...
    if (!studentNumber.trim() || !password) {
      setError("Enter your student number and password.");
      return;
    }
    setSubmitting(true);
    ...
    if (error)
      setError(
        /invalid/i.test(error)
          ? "That student number and password don't match. Try again, or tap Create account below."
          : error,
      );
  ...
  async function handleStudentSignUp(e: React.FormEvent) {
    ...
    if (!fullName.trim() || !studentNumber.trim() || !programmeCode || password.length < 6) {
      setError(
        "Fill in your name, student number, programme, and a password (at least 6 characters).",
      );
      return;
    }
    ...
    <div className="grid min-h-screen bg-background md:grid-cols-2">
      <div className="relative hidden overflow-hidden bg-hero md:block">
        <div
          ...
          style={{
            backgroundImage: `url(${campusGate.url})`,
        ...
        <div className="relative flex h-full flex-col justify-between p-10 text-primary-foreground">
          ...
          <div>
            ...
            <p className="mt-2 max-w-xs text-primary-foreground/75">
              Everything you need, organised by programme and year.
            </p>
          </div>
        </div>
      </div>
              ...
              {mode === "student-signup" ? (
                <form onSubmit={handleStudentSignUp} className="mt-6 space-y-3">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground shadow-soft transition-colors hover:bg-primary/90 disabled:opacity-60"
                  >
...
function GoogleGlyph() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path
        fill="#4CAF50"
        d="M24 44c5.1 0 9.7-1.9 13.2-5.1l-6.1-5c-2 1.4-4.5 2.1-7.1 2.1-5.3 0-9.7-3.4-11.3-8l-6.5 5C9.6 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.2-4.2 5.6l6.1 5c-.4.4 6.8-5 6.8-14.6 0-1.2-.1-2.3-.4-3.5z"
      />
    </svg>
  );
}
