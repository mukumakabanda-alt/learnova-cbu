...
export type DocumentVisualFinding = {
  ...
  confidence: number;
  source: "puter-vision" | "puter-ocr" | "native";
  sourceRef?: string;
...
const CLASSIFICATION_RULES: Array<{
...
}> = [
  {
    ...
    weight: 2,
    evidence: [
      [/(?:test|examination|exam|mid[- ]?semester|final|past paper)/i, "exam/test wording"],
      [/(?:answer all questions|time allowed|marks?)/i, "candidate instructions or marks"],
      [/(?:section\s+[a-d]|question\s*\d+|\d+[.)]\s+)/i, "question or section structure"],
      [/(?:multiple choice|choose one|candidate)/i, "assessment wording"],
    ],
  },
  {
    type: "Assignment",
    weight: 2,
    evidence: [
      [/(?:assignment|submit|submission|coursework|deadline|task)/i, "assignment wording"],
  ...
  },
  {
    ...
    weight: 2,
    evidence: [
      [/course\s+outline/i, "course-outline heading"],
      [/(?:learning outcomes?|objectives?|weekly topics?|course content)/i, "course structure wording"],
    ],
  },
  {
    type: "Slides",
    weight: 1.5,
    evidence: [
      [/slide\s*\d+/i, "slide boundaries"],
      [/(?:lecture|presentation|powerpoint)/i, "presentation wording"],
    ],
  },
  {
    type: "Notes",
    weight: 1,
    evidence: [
      [/(?:definition|example|therefore|introduction|conclusion)/i, "lecture-style explanation"],
    ],
  },
];
...
function shortId(prefix: string, unitIndex: number, itemIndex: number): string {
...
export function classifyAcademicDocument(
  ...
  const source = `${format} ${text}`.slice(0, 40_000);

  if (/\b(?:pptx?|powerpoint|slides?)\b/i.test(format)) {
    return {
      detectedType: "Slides",
      confidence: 0.96,
      evidence: ["PowerPoint/slide file format"],
    };
  }
  ...
  const confidence = Math.max(
    ...
    Math.min(
      0.99,
      0.45 + winner[1].score * 0.08 - secondScore * 0.03,
    ),
  );
...
function classifyLine(line: string): DocumentBlockKind {
  if (
    /^(question\s+\d+[\w.-]*|\d+[.)]\s+|[a-z][.)]\s+)/i.test(line)
  ) {
    return "question";
  }
  ...
  if (/^(?:figure|fig\.?|diagram|chart|graph)\b/i.test(line)) {
    return "figure";
  }
...
function qualitySignals(text: string) {
  ...
  const repeated = Array.from(counts.values())
    ...
    .reduce(
      (sum, count) => sum + count - 12,
      0,
    );

  return {
    abnormalCharacterRatio: chars.length
      ? abnormal / chars.length
      : 1,
    repeatedTokenRatio: words.length
...
export function buildAcademicDocumentModel(input: {
  ...
  const classification =
    input.documentType && input.documentType !== "unknown"
      ...
      : classifyAcademicDocument(
          input.units.map((unit) => unit.text).join("\n"),
          input.format,
        );

  const units: DocumentUnit[] = input.units.map((unit, index) => {
    const unitNumber = index + 1;
    const text = cleanBlockText(unit.text ?? "");
    const extraction = unit.extraction ?? "unknown";

    const confidence = bounded(
      unit.confidence,
      text.length >= 25 ? 0.9 : 0.15,
    );

    const blocks: DocumentBlock[] = text
      .split(/\n+/)
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line, sourceIndex) => ({
        id: shortId("b", unitNumber, sourceIndex),
    ...
    const visualFindings = (unit.visualFindings ?? []).filter(
      (finding) => finding.description.trim(),
    );

    const visualConfidence = visualFindings.length
      ? visualFindings.reduce(
          (sum, finding) =>
            sum + bounded(finding.confidence),
          0,
  ...
  const allBlocks = units.flatMap(
    (unit) => unit.blocks,
  );

  const visualFindings = units.flatMap(
    (unit) => unit.visualFindings,
  );
  ...
  const readableUnits = units.filter(
    (unit) =>
      unit.text.trim().length >= 20 ||
      unit.confidence >= 0.7 ||
      unit.visualFindings.some(
        (finding) =>
          bounded(finding.confidence) >= 0.65,
      ),
  ).length;

  const readableUnitRatio = units.length
    ? readableUnits / units.length
    : 0;

  const visualUnitRatio = units.length
    ? units.filter(
        (unit) =>
          unit.visualFindings.length > 0,
      ).length / units.length
    : 0;

  const averageUnitConfidence = units.length
    ? units.reduce((sum, unit) => {
        const visualBoost = unit.visualFindings.length
          ? Math.max(
              unit.visualConfidence * 0.25,
              0.05,
            )
          : 0;
        ...
        return (
          ...
          Math.min(
            1,
            unit.confidence + visualBoost,
          )
        );
      }, 0) / units.length
    : 0;

  const structureCount = allBlocks.filter(
  ...
  const abnormalPenalty = Math.min(
    0.20,
    signals.abnormalCharacterRatio * 1.25,
  );

  const repeatedPenalty = Math.min(
    0.10,
    signals.repeatedTokenRatio * 0.22,
  );

  const visualBonus = Math.min(
    0.10,
    visualUnitRatio * 0.08 +
      Math.min(
        0.02,
        visualFindings.length * 0.001,
      ),
  );
  ...
  const coverage = units.length
    ? Math.round(
        (units.filter(
          (unit) =>
            unit.text.trim().length >= 12 ||
            unit.confidence >= 0.65 ||
            unit.visualFindings.length > 0,
        ).length /
          units.length) *
          100,
  ...
  return {
    ...
    signals: {
      ...
      readableUnitRatio:
        Math.round(
          readableUnitRatio * 10_000,
        ) / 10_000,
      visualUnitRatio:
        Math.round(
          visualUnitRatio * 10_000,
        ) / 10_000,
      visualFindingCount:
        visualFindings.length,
      headingCount:
...
/**
 * Merge page/slide visual intelligence into an
...
export function mergeVisualFindings(
  ...
  for (const update of updates) {
    const current =
      updateMap.get(update.unitIndex) ?? [];

    current.push(...update.findings);
...
export function serializeDocumentForStudy(
  ...
  for (const unit of model.units) {
    ...
    for (const finding of unit.visualFindings) {
      const typeLabel =
